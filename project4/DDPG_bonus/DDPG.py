...
class OU_Noise:
    ...
    def _sample(self):
        ...
        # ========== YOUR CODE HERE ==========
        dx = self.theta * (self.mu - self.state) + self.sigma * np.random.normal(size=self.action_space)
        self.state += dx
        return self.state
    ...
    def noise(self, action: np.ndarray[float]):
        ...
        noised_action = np.clip(noised_action, self.action_range[0], self.action_range[1])
